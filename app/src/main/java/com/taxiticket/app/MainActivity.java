...
public class MainActivity extends Activity {
    ...
    private WebView createWebView() {
        ...
        v.setWebViewClient(new WebViewClient() {
            @Override public void onPageFinished(WebView view, String url) {
                view.evaluateJavascript("window.print=function(){TaxiTicketAndroid.print()};", null);
            }
        });
        ...
        v.setWebChromeClient(new WebChromeClient() {
            @Override public boolean onShowFileChooser(WebView view, ValueCallback<Uri[]> cb, FileChooserParams params) {
                ...
                if (captureRequested && ContextCompat.checkSelfPermission(MainActivity.this, Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
                    new android.app.AlertDialog.Builder(MainActivity.this)
                        ...
                        .setMessage("TaxiTicket necesita la cámara únicamente para fotografiar el ticket ahora. No se enviará la foto a ningún servidor.")
                        .setNegativeButton("Cancelar", (d,w) -> { if(fileCallback!=null){fileCallback.onReceiveValue(null);fileCallback=null;} })
                        .setPositiveButton("Continuar", (d,w) -> ActivityCompat.requestPermissions(MainActivity.this,new String[]{Manifest.permission.CAMERA},REQ_CAMERA)).show();
            ...
            @Override public boolean onCreateWindow(WebView view, boolean dialog, boolean userGesture, Message resultMsg) {
                child=createWebView(); root.addView(child,new FrameLayout.LayoutParams(-1,-1));
                WebView.WebViewTransport t=(WebView.WebViewTransport)resultMsg.obj; t.setWebView(child); resultMsg.sendToTarget(); return true;
            }
        }); return v;
    }
    private void launchPicker() {
        Intent docs=new Intent(Intent.ACTION_OPEN_DOCUMENT); docs.addCategory(Intent.CATEGORY_OPENABLE); docs.setType("image/*");
        Intent chooser=Intent.createChooser(docs,"Selecciona o haz una foto del ticket");
        if (captureRequested) {
            Intent camera=new Intent(MediaStore.ACTION_IMAGE_CAPTURE);
            try { File dir=new File(getCacheDir(),"images"); if(!dir.exists())dir.mkdirs();
                File f=File.createTempFile("ticket_", ".jpg", dir);
                cameraUri=FileProvider.getUriForFile(this,"com.taxiticket.app.fileprovider",f);
    ...
    private static class PrintBridge {
        private final WebView view; PrintBridge(WebView v){view=v;}
        @android.webkit.JavascriptInterface public void print(){
            PrintManager pm=(PrintManager)view.getContext().getSystemService(Context.PRINT_SERVICE);
            if(pm!=null) pm.print("TaxiTicket", view.createPrintDocumentAdapter("TaxiTicket"), new PrintAttributes.Builder().setMediaSize(PrintAttributes.MediaSize.ISO_A4).build());
        }
    }
}
